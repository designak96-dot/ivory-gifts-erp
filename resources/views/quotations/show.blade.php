...
@endif
<div class="card" style="margin-top:18px"><h2>Version history</h2>
@php($latestVersion=$quotation->versions->max('version_number'))
<div class="table-wrap" style="margin-top:15px"><table><thead><tr><th>Version</th><th>Created by</th><th>Date</th><th></th></tr></thead><tbody>
@forelse($quotation->versions->sortByDesc('version_number') as $v)
<tr><td><b>Version {{ $v->version_number }}</b> @if($v->version_number===$latestVersion)<span class="badge green">Current</span>@endif</td><td>{{ \App\Models\User::find($v->created_by)?->name??'System' }}</td><td>{{ $v->created_at->format('d M Y, h:i A') }}</td><td><a class="btn small" href="{{ route('quotations.version',[$quotation,$v->version_number]) }}">View</a></td></tr>
@empty
