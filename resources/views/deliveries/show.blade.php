...
@if(in_array($order->simple_status,['ready','delivered']))<button type="button" class="btn small success" data-whatsapp-share data-whatsapp-status="{{ $order->simple_status }}" data-check-url="{{ route('whatsapp.check',$order) }}" data-link-url="{{ route('whatsapp.link',$order) }}">@include('partials._whatsapp-icon') WhatsApp</button>@endif
